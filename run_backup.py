    ...
    def run(self):
        ...
        while True:
            ...
            try:
                root = tkinter.Tk()
                root.withdraw()
                if FLAG_RUNNING_STATUS:
                    upload.online_report()
                    root.after(sec_timer*1000, root.destroy)
                    tkinter.messagebox.showinfo(title='Device Status', message='Connected successfully')
                    
                else:
                    camera_ip_l = mac.get_mac()
                    if not len(camera_ip_l):
                        upload.error_report()
                        FLAG_RUNNING_STATUS = False
                        root.after(sec_timer*1000, root.destroy)
                        tkinter.messagebox.showinfo(title='Device Status', message='No camera!')
...
def multi_camera():
    ...
    try:
        ...
        for queue, camera_ip in zip(queues, camera_ip_l):
            ...
            processes.append(mp.Process(target=frame_get, args=(queue, camera_ip)))
